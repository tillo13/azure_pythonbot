...
async def search_general(query):  # Renamed function  
    ...
    # Combine all valid results into one text  
    combined_text = " ".join(result['content'] for result in valid_results)  
  
    # Chunk the combined text to fit within the 2000-character limit  
    max_chunk_size = 2000  
    chunks = chunk_text(combined_text, max_chunk_size)  
  
    # Generate a final summary within the 2000-character limit  
    final_summary = ""  
    for chunk in chunks:  
        messages = [  
            {"role": "system", "content": "You are a helpful assistant that summarizes web content."},  
            {"role": "user", "content": f"Summarize the following information in 2000 characters or less: {chunk}"}  
        ]  
    ...
    if not final_summary:  
        return "Could not generate a summary for the given query."  
